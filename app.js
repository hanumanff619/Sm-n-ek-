...
let NAMEDAYS = {};
fetch('namedays_cs.json?v=2.0',{cache:'no-store'}).then(r=>r.json()).then(j=>{ NAMEDAYS=j; updateToday(); }).catch(_=>{});

function setBg(){
  const url = S.mode==='8' ? 'backgrounds/bg8.jpg' : 'backgrounds/bg12.jpg';
...
function recompute(){
  ...
  const gross = basePay + payAft + payNight + payWend + payHol + payCont + mgr + yearBonus;
...
document.getElementById('btnSave').onclick = ()=>{
  ...
  const auto = (S.avg>0) ? S.avg : (function(){
    const sumNet = (S.m1_net||0)+(S.m2_net||0)+(S.m3_net||0);
    const sumHrs = (S.m1_hrs||0)+(S.m2_hrs||0)+(S.m3_hrs||0);
    return (sumNet>0 && sumHrs>0) ? sumNet/sumHrs : 0;
  })();
  if(!S.avg && auto>0){ S.avg = auto; }
