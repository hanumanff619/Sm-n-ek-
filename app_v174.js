// Směnářek 1.7.4 – CZ svátky (celý rok, včetně pohyblivých), Cafeterie checkbox 1000 Kč (mimo čistou)
...
let state = JSON.parse(localStorage.getItem('smenarek_state_v174')||'{}');
...
if(state.cafeteria_ok==null) state.cafeteria_ok=false; // checkbox
...
function isHoliday(dt){
  const y=dt.getFullYear(); const key=pad(dt.getMonth()+1)+'-'+pad(dt.getDate());
  return czechHolidays(y).has(key);
}

// Nameday placeholder do hlavičky (ne kompletní seznam – jen aby nebylo prázdno)
const NAMEDAYS_EMBED = {"10":{"01":"Igor","02":"Olivie","03":"Bohumil","04":"František","05":"Eliška","06":"Hanuš","07":"Justýna","08":"Věra","09":"Štefan","10":"Marina","11":"Andrej","12":"Marcel","13":"Renáta","14":"Agáta","15":"Tereza","16":"Havel","17":"Hedvika","18":"Lukáš","19":"Michaela","20":"Vendelín","21":"Brigita","22":"Sabina","23":"Teodor","24":"Nina","25":"Beáta","26":"Erik","27":"Šarlota","28":"Státní svátek","29":"Silvie","30":"Tadeáš","31":"Štěpánka"}};
function localNameday(date){const mm=pad(date.getMonth()+1), dd=pad(date.getDate()); return NAMEDAYS_EMBED[mm]?.[dd]||null;}

function save(){ localStorage.setItem('smenarek_state_v174', JSON.stringify(state)); }
...
function updateStats(){
  ...
  for(let i=1;i<=last.getDate();i++){
    ...
    if(state.mode==='8'){
      ...
      if(isHoliday(dt) && t!=='V') holWorkedH+=8;
      const next=new Date(y,m,i+1);
      if(t==='N' && isHoliday(next)) holWorkedH+=6;

    } else {
      if(t==='D'){ dDay++; hours+=DAILY; afterH+=4.0; if(isW(dt)) weekendH+=DAILY; if(isHoliday(dt)) holWorkedH+=DAILY; }
      if(t==='N'){
        nDay++; hours+=DAILY;
        afterH += 4.25;
        nightH += 8.0;
        ...
        if(isHoliday(dt)) holWorkedH+=DAILY;
        const next=new Date(y,m,i+1);
  ...
  state._calc={hours,afterH,nightH,weekendH,vac,holWorkedH,DAILY,H8,VAC12:11.25,VAC8:8.0}; save();
}
...
function calcPay(){
  const avg=avgRate();
  const C=state._calc||{hours:0,afterH:0,nightH:0,weekendH:0,vac:0,holWorkedH:0,DAILY:12.25,H8:8.0,VAC12:11.25,VAC8:8.0};
  const r={
