// Směnářek 1.7.8 – silnější pozadí kalendáře, auto highlight dne, sjednocené nápovědy, bez přepínače pozadí
...
let state = JSON.parse(localStorage.getItem('smenarek_state_v178')||'{}');
...
if(state.bgEnabled==null) state.bgEnabled=true; // kept internally, but no toggle
...
function save(){ localStorage.setItem('smenarek_state_v178', JSON.stringify(state)); }
...
function calcPay(){
  ...
  const nightPay= noc * C.nightH;
  const wkPay   = vikend * C.weekendH;
