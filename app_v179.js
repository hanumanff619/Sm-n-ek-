// Směnářek 1.7.9 – help icons fixed, nameday header, results split, bg & today highlight
...
let state = JSON.parse(localStorage.getItem('smenarek_state_v179')||'{}');
...
// Nameday (basic inline table; can be expanded)
const NAMEDAYS = {"01-01": "Nov\u00fd rok", "01-02": "Karina", "01-03": "Radmila", "01-04": "Diana", "01-05": "Dalimil", "01-06": "T\u0159i kr\u00e1lov\u00e9", "01-07": "Vilma", "01-08": "\u010cestm\u00edr", "01-09": "Vladan", "01-10": "B\u0159etislav", "09-28": "V\u00e1clav", "10-28": "St\u00e1tn\u00ed sv\u00e1tek", "12-24": "\u0160t\u011bdr\u00fd den", "12-25": "Bo\u017e\u00ed hod", "12-26": "\u0160t\u011bp\u00e1n"};

function todayNameday(){
  const d=new Date(); const key = pad(d.getMonth()+1)+'-'+pad(d.getDate());
  return NAMEDAYS[key] || '—';
}

function save(){ localStorage.setItem('smenarek_state_v179', JSON.stringify(state)); }
...
function updateHeader(){
  ...
  $('todayNameday').textContent = 'Svátek: ' + todayNameday();
}
...
function updateStats(){
  ...
  const DAILY_WORKED = 12.25;  // reálné odpracované u 12h směny
  const VAC12 = 11.25;         // náhrady (svátek/dovolená) u 12h režimu
  const H8 = 8.0;              // 8h směny
...
function calcPay(){
  ...
  const holPay  = avg * C.holWorkedH;           // svátek via 11.25h v 12h režimu
