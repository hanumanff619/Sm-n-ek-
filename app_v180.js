// Směnářek 1.8.0 – namedays offline file + help icon alignment
...
let state = JSON.parse(localStorage.getItem('smenarek_state_v180')||'{}');
...
let current=new Date(), selectedDate=null, NAMEDAYS=null;
...
async function loadNamedays(){
  if(NAMEDAYS) return NAMEDAYS;
  try{
    const res = await fetch('namedays_cz.json');
    NAMEDAYS = await res.json();
  }catch(e){
    NAMEDAYS = {};
  }
  return NAMEDAYS;
...
function todayNameday(){
  const d=new Date(); const key = md(d);
  return (NAMEDAYS && NAMEDAYS[key]) ? NAMEDAYS[key] : '—';
}

function save(){ localStorage.setItem('smenarek_state_v180', JSON.stringify(state)); }
...
async function renderCalendar(){
  ...
  for(let r=0;r<6;r++){
    ...
    for(let c=0;c<7;c++){
      ...
      const name = (NAMEDAYS && NAMEDAYS[md(dt)] && NAMEDAYS[md(dt)]!=='—') ? ' 🟦' : '';
      html+=`<td data-date="${key}" class="${classes.join(' ')}">
               <div class="daynum">${day}${isHoliday(dt)?' 🎌':''}${name}</div>
...
function calcPay(){
  ...
  const holPay  = avg * C.holWorkedH;    // svátky přes 11.25h v 12h režimu
