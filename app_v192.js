// Směnářek 1.9.2 (stable) – pevný kalendář, svátky + jmeniny (dnes), fix motivační 6/11, info vpravo
const MEAL_DEDUCT = 40, LUNCH_DEDUCT = 40, MEAL_INFO_VALUE = 110;

let state = JSON.parse(localStorage.getItem('smenarek_state')||'{}');
if(!state._schema) state._schema = 1;
...
if(state.annual_june==null) state.annual_june=0;
if(state.annual_nov==null) state.annual_nov=0;
if(state.leader_fund==null) state.leader_fund=0;

// MIGRACE -> stravenky/obědy/skip
if(state._schema < 2){
  if(state.meal_auto == null) state.meal_auto = true;
  if(state.meal_count == null) state.meal_count = 0;
  if(state.lunch_count == null) state.lunch_count = 0;
  if(state.lunch_skip == null) state.lunch_skip = 0;
  state._schema = 2;
  localStorage.setItem('smenarek_state', JSON.stringify(state));
}
...
const nval=v=>(+v)||0;
const money=x=>(Math.round((x||0)*100)/100).toLocaleString('cs-CZ',{style:'currency', currency:'CZK'});

// nameday (today only – spolehlivé)
...
// státní svátky CZ (fix + pohyblivé)
const HOLI_CACHE = {};
...
function updateHeader(){
  const today = new Date(); const key=ymd(today);
...
function bindInputsOnce(){
  ...
  $('annual_june').value = state.annual_june||0;
  $('annual_june').oninput = ()=>{ state.annual_june=nval($('annual_june').value); save(); calcPay(); };
  $('annual_nov').value = state.annual_nov||0;
  $('annual_nov').oninput = ()=>{ state.annual_nov=nval($('annual_nov').value); save(); calcPay(); };
  $('leader_fund').value = state.leader_fund||0;
  $('leader_fund').oninput = ()=>{ state.leader_fund=nval($('leader_fund').value); save(); calcPay(); };

  $('meal_auto').checked = !!state.meal_auto;
  $('meal_auto').onchange = ()=>{ state.meal_auto = $('meal_auto').checked; save(); calcPay(); };
  $('meal_count').value = state.meal_count;
  $('meal_count').oninput = ()=>{ state.meal_count = Math.max(0, Math.floor(nval($('meal_count').value))); save(); calcPay(); };
  $('lunch_count').value = state.lunch_count;
  $('lunch_count').oninput = ()=>{ state.lunch_count = Math.max(0, Math.floor(nval($('lunch_count').value))); save(); calcPay(); };
  $('lunch_skip').value = state.lunch_skip;
  $('lunch_skip').oninput = ()=>{ state.lunch_skip = Math.max(0, Math.floor(nval($('lunch_skip').value))); save(); calcPay(); };
...
function updateStats(){
  ...
  const DAILY_WORKED = 11.25; // 12h směna účtovaně
  const VAC12 = 11.25, H8 = 8.0;
...
function calcPay(){
  ...
  // Motivační – jen v červnu (6) a listopadu (11)
  const month = current.getMonth() + 1;
  const motJun = (month === 6)  ? (state.annual_june || 0) : 0;
  const motNov = (month === 11) ? (state.annual_nov  || 0) : 0;
  const leaderFund = state.leader_fund || 0;

  // stravenky/obědy (auto/ručně + skip)
  function mealsCalcAuto(){
    ...
    for(let i=1;i<=end.getDate();i++){
      ...
      if(state.mode==='12'){
        if(t==='N'){ count += 2; }
        if(t==='D'){ if(isW(dt)) count += 2; else { count += 1; lunches += 1; } }
      }else{
        if(t==='N'){ count += 2; }
        if(t==='R' || t==='O'){ if(isW(dt)) count += 2; else { count += 1; lunches += 1; } }
      }
    }
    return {count, lunches};
  }
  const auto = mealsCalcAuto();
  let mCount = state.meal_auto ? auto.count   : (state.meal_count||0);
  let lCount = state.meal_auto ? auto.lunches : (state.lunch_count||0);
  lCount = Math.max(0, lCount - (state.lunch_skip||0));

  const mealDeduct  = mCount * MEAL_DEDUCT;
  const lunchDeduct = lCount * LUNCH_DEDUCT;
  const mealValue   = mCount * MEAL_INFO_VALUE;

  const gross = basePay+odpoPay+nightPay+wkPay+holPay+nepret+prime+vacPay
              + (state.annual_bonus||0) + motJun + motNov + leaderFund;

  const rndDown = x => Math.floor(x);
  const rndUp   = x => Math.ceil(x);
  const up100   = x => Math.ceil(x / 100) * 100;

  const social = rndDown(gross * 0.065);
  const health = rndUp(gross * 0.045);
  const taxBase = up100(gross - social - health);
  ...
  $('pay').innerHTML = [
    ...
    ['Motivační – červen', money(motJun)],['Motivační – listopad', money(motNov)],
    ['Prémie z fondu vedoucího', money(leaderFund)],
    ['Srážka stravenky ('+mCount+' ks)','− '+money(mealDeduct)],
    ['Srážka obědy ('+lCount+' ks)','− '+money(lunchDeduct)]
